    ...
    public class UpdateAndFilterTests(CustomWebApplicationFactory factory) : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        public async Task FilterDocuments_ShouldReturnMatchingDocuments()
        {
            ...
            var docs = new[]
            {
                ...
                new { title="B", product="P2", team="T1", author="Bob", taskLink="", content="#B", status="UnderReview", gitRepository="/tmp/repo", gitFilePath="b.md", gitCommitHash="c" },
            ...
            // Filter by status UnderReview
            var byStatus = await _client.GetFromJsonAsync<List<DocumentSummary>>("/api/documents?status=UnderReview");
            Assert.NotNull(byStatus);
            Assert.Single(byStatus!);
            Assert.Equal("B", byStatus.First().Title);

            // Pagination: pageSize=2 should return 2 docs on first page and 1 on second
            var page1 = await _client.GetFromJsonAsync<List<DocumentSummary>>("/api/documents?page=1&pageSize=2");
            Assert.NotNull(page1);
            Assert.Equal(2, page1!.Count);

            var page2 = await _client.GetFromJsonAsync<List<DocumentSummary>>("/api/documents?page=2&pageSize=2");
